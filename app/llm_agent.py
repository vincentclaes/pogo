...
def _record_sql_step(
    ...
    deps.recorder.append_result_preview(_df_preview(df))

    plots = generate_plots(df, deps.out_dir / "plots", start_index=deps.plot_counter)
    deps.plot_counter += len(plots)

    for plot in plots:
        caption = viz_caption or f"Chart type: {plot.chart_type}"
...
def build_llm_agent(model_name: str = DEFAULT_MODEL) -> Agent[AgentDeps, AgentDecision]:
    system_prompt = (
        "You are a data analyst agent for biosignal.\n"
    ...
    def run_sql(
        ...
        plot_paths = _record_sql_step(ctx, sql, df, reasoning_title, reasoning, viz_caption)
        payload = {
            "rows": _df_preview(df),
            "row_count": len(df),
